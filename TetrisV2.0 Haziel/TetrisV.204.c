/* * DESARROLLO DE TETRIX EN LENGUAJE DE PROGRAMACION C
-----------------------------------------------------------------------------------------------------------------------------------------------------------------
   * MATERIA: PROGRAMACION ESTRUCTURADA
-----------------------------------------------------------------------------------------------------------------------------------------------------------------
   * PROFESOR: J. JESUS ARRELLANO PIMENTEL
-----------------------------------------------------------------------------------------------------------------------------------------------------------------
   * ALUMNO: EDGAR HAZIEL REYNA GARCIA
------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
void trazartablero(HDC, int, int, int [NRTA][NCTA], HBRUSH hbr);
void inicializartablero(int [NRTA][NCTA]);
void imprimirNombre(HDC, int , int); // IMPRIMIR NOMBRE
void trazarNombre(HDC, int, int); // NOMBRE
	...
	switch(Message) {
		...
		case WM_KEYDOWN:{
			switch(wParam)
			{
				...
				case VK_SPACE:{
					PlaySound("sound-aceptar.wav", NULL, SND_FILENAME|SND_ASYNC);
					SetTimer(hwnd, IDT_TIMER1, 1000000, NULL);
		...
		case WM_PAINT:{
			...
			TextOut(hdc, 850, 70, szMessage, strlen(szMessage));
			
			wsprintf(ssszMessage, "Puntuacion Maxima:  %d", renElimx);
			TextOut(hdc, 820, 36, ssszMessage, strlen(ssszMessage));
		
		// COLOR DE FONDO //
			SelectObject(hdc, hbrArre[10]);
			Rectangle(hdc, 0, 0, 1080, 680);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","TetrisV2.204",WS_VISIBLE|WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT, /* x */
		CW_USEDEFAULT, /* y */
		1080, /* width */
		760, /* height */
...
void trazarcuadrito(HDC hdc, int x, int y, int i, int j, HBRUSH hbrElem)
{
	RECT r;
	...
	FillRect(hdc, &r, hbrElem);
}

void trazartablero(HDC hdc, int x, int y, int tab[NRTA][NCTA], HBRUSH hbrElem)
{
	int i = 0, j = 0;
	...
	for(i = 0; i < NRTA;i++){
		for(j = 0; j < NCTA;j++){
			if(tab[i][j]){
				trazarcuadrito(hdc, x, y, i, j, hbrElem);
			}
		}
	}
...
void trazarElemento(HDC hdc, int x, int y, STELEMENTO elem, HBRUSH *hbrElem)
{
	int i = 0, j = 0;

	for( i = 0; i < NRET;i++){
		for(j = 0; j < NCET;j++){
			if(elem.arre[i][j]){ // en 1 es verdadero y en 0 es falso
			trazarcuadrito(hdc, x, y, elem.ren+i, elem.col+j, *hbrElem);
			}
		}
	}
}
...
int moverElemento(int tab[NRTA][NCTA], STELEMENTO *elem, int dir)
{
	if(!validarMovimiento(tab, *elem, dir)){
		PlaySound("sound-aceptar.wav", NULL, SND_FILENAME|SND_ASYNC); // SONIDO PARA CUANDO SE COMPLETA UNA LINEA
		return 0;
	}
	switch(dir)
	{
...
void asignar_Elemento(STELEMENTO elem,  int tab[NRTA][NCTA])
{
	int i = 0, j = 0;
		...
		for(i = 0; i < NRET ; i++){
			for(j = 0; j < NCET; j++){
				if(elem.arre[i][j]){
				//	PlaySound("sound-aceptar.wav", NULL, SND_FILENAME|SND_ASYNC);  SONIDO PARA CUANDO ENTRE EN CONTACTO CON ALGO //
...
void trazarNombre(HDC hdc, int x, int y)
{
	RECT r;
...
void imprimirNombre(HDC hdc, int x, int y)
{
	// LETRA H  //
	trazarNombre(hdc, 80, 120);  trazarNombre(hdc, 80, 160);   trazarNombre(hdc, 80, 200);  trazarNombre(hdc, 80, 240);  trazarNombre(hdc, 80, 280);
	trazarNombre(hdc, 160, 120); trazarNombre(hdc, 160, 160);  trazarNombre(hdc, 160, 200); trazarNombre(hdc, 160, 240); trazarNombre(hdc, 160, 280);
	trazarNombre(hdc, 120, 200);
	
	// LETRA R //
	trazarNombre(hdc, 320, 120); trazarNombre(hdc, 320, 160); trazarNombre(hdc, 320, 200); trazarNombre(hdc, 320, 240); trazarNombre(hdc, 320, 280);
	trazarNombre(hdc, 360, 120); trazarNombre(hdc, 400, 160); trazarNombre(hdc, 360, 200); trazarNombre(hdc, 400, 240); trazarNombre(hdc, 400, 280);
	trazarNombre(hdc, 400, 120);
	// LETRA G //
	trazarNombre(hdc, 560, 120); trazarNombre(hdc, 560, 160); trazarNombre(hdc, 560, 200); trazarNombre(hdc, 560, 240); trazarNombre(hdc, 560, 280);
	trazarNombre(hdc, 600, 280); trazarNombre(hdc, 640, 280); trazarNombre(hdc, 640, 200); trazarNombre(hdc, 680, 200); trazarNombre(hdc, 680, 240);
	trazarNombre(hdc, 600, 120); trazarNombre(hdc, 640, 120);  trazarNombre(hdc, 680, 280);
}
